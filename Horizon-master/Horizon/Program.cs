...
namespace Horizon
{
    public class Program
    {
        ...
        //public List<Requirement> requirementList = new List<Requirement>(); //I ADDED THIS
        List<XmlNode> ReqNodes = new List<XmlNode>(); //I ADDED THIS
        ...
        public Requirement sysRequirement = new Requirement();
        public HSFTranslator hsfTranslator = new HSFTranslator();
        public int tot_num_asset = 0; //I ADDED THIS
        public bool reqswitch = false; //I ADDED THIS
        ...
        static int Main(string[] args)
        {
            ...
            //SysML File Check
            Console.WriteLine("Conducting Check for SysML File");

            bool isSysML = false; //Default designation if file is SysML format
            
            isSysML = program.hsfTranslator.SysMLCheck(program.modelInputFilePath);
            Console.WriteLine(isSysML);
            if (isSysML)
            {
                string SysMLPath = Directory.GetCurrentDirectory();
                Console.WriteLine("SysML File Detected. Converting to HSF Format...");
                program.modelInputFilePath = program.hsfTranslator.TranslateSysML(program.modelInputFilePath, SysMLPath);
                Console.WriteLine("Conversion Complete.");
            }
            else
            {
                Console.WriteLine("SysML File Not Detected. Proceeding with original file format.");
            }
            ...
            if (program.reqswitch == true)
            {
                program.EvaluateRequirements(); //Execute evaluation here
            }
        ...
        {
            // Set Defaults
            simulationInputFilePath = @"..\..\..\SimulationInput_Extended.XML"; //I CAN CHANGE THIS
            targetDeckFilePath = @"..\..\..\v2.2-300targets.xml"; //I CAN CHANGE THIS
            modelInputFilePath = @"..\..\..\HSF_Aeolus_Model_R2.xml"; //I CAN CHANGE THIS
        ...
        public void LoadSubsystems()
        {
            ...
            foreach (XmlNode modelChildNode in modelInputXMLNode.ChildNodes)
            {
                ...
                if (modelChildNode.Name.Equals("REQUIREMENT"))
                {
                    reqswitch = true;
                    ReqNodes.Add(modelChildNode);
                }
        ...
        {
            foreach (XmlNode ReqNodeItem in ReqNodes)
            {

                //Run Python script to evaluate requirement
                Requirement Req1 = new Requirement();
                Req1.AnalyzeRequirement(ReqNodeItem, tot_num_asset);
                Req1 = null;
                //sysRequirement.AnalyzeRequirement(ReqNodeItem, tot_num_asset);
                //Why isn't AnalyzeRequirement recognized by HorizonMain?!?!?!
